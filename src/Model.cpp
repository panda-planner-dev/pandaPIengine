        ...
        *sStream >> numActions;
        ...
        for (int i = 0; i < numActions; i++) {
            ...
            *sStream >> actionCosts[i];
            ...
            iu.sort(precLists[i], 0, numPrecs[i] - 1);
            if (numPrecs[i] == 0) {
                numPrecLessActions++;
            }
...
bool Model::taskReachable(searchNode* tn, int t) {
    ...
    void Model::generateProducerConsumerLists() {
        /*
        set<int> *tAP = new set<int>[this->numStateBits]();
        set<int> *tSP = new set<int>[this->numStateBits]();
        set<int> *tAC = new set<int>[this->numStateBits]();
        set<int> *tSC = new set<int>[this->numStateBits]();

        for (int ia = 0; ia < numActions; ia++) {
            if (!iu.isSorted(precLists[ia], numPrecs[ia])) {
                for (int i = 0; i < numPrecs[ia]; i++)
                    cout << precLists[ia][i] << " ";
            }
            assert(iu.isSorted(precLists[ia], numPrecs[ia]));
            assert(iu.isSorted(addLists[ia], numAdds[ia]));
            assert(iu.isSorted(delLists[ia], numDels[ia]));

            for (int ip = 0; ip < numPrecs[ia]; ip++) {
                int fp = precLists[ia][ip];
                if (iu.containsInt(delLists[ia], 0, numDels[ia] - 1, fp)) {
                    tAC[fp].insert(ia);
                }
                // todo SAS+ stuff
            }
        ...
        int *numAP = new int[numStateBits];
        int *numSP = new int[numStateBits];
        int *numAC = new int[numStateBits];
        int *numSC = new int[numStateBits];

        int **apList = new int *[numStateBits];
        int **spList = new int *[numStateBits];
        int **acList = new int *[numStateBits];
        int **scList = new int *[numStateBits];
        ...
        for (int i = 0; i < numStateBits; i++) {
            ...
            numAC[i] = tAC[i].size();
            numSC[i] = tSC[i].size();
            apList[i] = new int[numAP[i]];
            spList[i] = new int[numSP[i]];
            acList[i] = new int[numAC[i]];
            scList[i] = new int[numSC[i]];
            int j = 0;
            for (int f : tAP[i]) apList[i][j++] = f;
            j = 0;
            for (int f : tSP[i]) spList[i][j++] = f;
            j = 0;
            for (int f : tAC[i]) acList[i][j++] = f;
            j = 0;
            for (int f : tSC[i]) scList[i][j++] = f;
        }
        delete[] tAP;
        delete[] tSP;
        delete[] tAC;
