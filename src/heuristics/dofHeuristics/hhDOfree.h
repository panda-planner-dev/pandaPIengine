...
namespace progression {
    ...
    enum csAddExternalLms {
        csAddExternalLmsYes, csAddExternalLmsNo
    };
    enum csLmcLms {
    ...
    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    class hhDOfree {
    ...
    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    hhDOfree<TINT, TBOOL>::hhDOfree(Model *htn, searchNode *n) :
            cTdg(cTdgFull),
            cPg(cPgFull),
    ...
    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    void hhDOfree<TINT, TBOOL>::printHeuristicInformation(Model *htn) {
        string s = "dof";
    ...
    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    void hhDOfree<TINT, TBOOL>::setHeuristicValue(searchNode *n, searchNode *parent,
                                                  int action) {
    ...
    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    void hhDOfree<TINT, TBOOL>::updatePG(searchNode *n) {
    ...
    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    void hhDOfree<TINT, TBOOL>::setHeuristicValue(searchNode *n, searchNode *parent, int absTask,
                                                  int method) {
        this->setHeuristicValue(n, parent, -1);
    }

    template<IloNumVar::Type TINT, IloNumVar::Type TBOOL>
    int hhDOfree<TINT, TBOOL>::recreateModel(searchNode *n) {
        ...
        for (int i = pg->usefulFactSet.getFirst(); i >= 0;
            ...
            v.add(IloNumVar(lenv, 0, 1, TBOOL));
            iUF[i] = iv;
#ifdef NAMEMODEL
        ...
        for (int i = pg->reachableTasksSet.getFirst(); i >= 0;
            ...
            v.add(IloNumVar(lenv, 0, INT_MAX, TINT));
            iUA[i] = iv;
#ifdef NAMEMODEL
        ...
        if ((this->cPg == cPgTimeRelaxed) || (this->cPg == cPgFull)) {
            for (int i = pg->usefulFactSet.getFirst(); i >= 0;
                 i = pg->usefulFactSet.getNext()) {
                v.add(IloNumVar(lenv, 0, htn->numActions, TINT));
                iTP[i] = iv;
#ifdef NAMEMODEL
