...
std::string to_string(Model *htn) {
  ...
  for (int v = 0; v < vertex_names.size(); v++)
    ...
    // "\"];\n";
    s += "  v" + std::to_string(v) + "[label=\"" +
         std::to_string(vertex_to_method[v]) + "\"];\n";
  ...
  for (int v = 0; v < vertex_names.size(); v++)
    for (auto &[task, tos] : edges[v])
      for (auto &[to, bdd] : tos)
        s += "  v" + std::to_string(v) + " -> v" + std::to_string(to) +
             " [label=\"" +
             // htn->taskNames[task] + "\"]\n";
             std::to_string(task) + "\"]\n";
...
void build_automaton(Model *htn) {
  ...
  sym_vars.init(false); // => No double primed vars
  ...
  for (int i = 0; i < htn->numActions; ++i) {
    ...
    trs.back().init(htn);
    // sym_vars.bdd_to_dot(trs.back().getBDD(), "op" + std::to_string(i) +
    // ".dot");
  }
...
#define put push_back
  ...
  while (cq.size() || prim_q.size() || abst_q.size()) {
    if (cq.size() == 0) {
      ...
      double layer_time_in_ms =
          1000.0 * (layer_end - layer_start) / CLOCKS_PER_SEC;
      std::cout << "Layer time: " << layer_time_in_ms << "ms" << std::endl
                << std::endl;
      ...
      if (current_abstract) {
        ...
        cq = abst_q;
        abst_q.clear();
        std::cout << "========================== Abstracts Depth " << depth
                  << std::endl;
      }
    ...
    if (step % 1000 == 0)
      std::cout << "STEP #" << step << ": " << task << " "
                << vertex_to_method[to] << std::endl;

    if (task < htn->numActions) {
      // apply action to state
      BDD nextState = trs[task].image(state);
      // sym_vars.bdd_to_dot(nextState, "nextstate" + std::to_string(step) +
      // ".dot");
        ...
        for (auto &[task2, tos] : edges[to])
          for (auto &[to2, bdd] : tos) {
            ...
            if (edgeDisjunct != edges[0][task2][to2]) {
              ...
              // std::cout << "\tPrim: " << task2 << " " <<
              // vertex_to_method[to2] << std::endl;
              addQ(task2, to2);
            }
          }

        if (to == 1) {
          std::cout << "Goal reached! Length=" << depth << " steps=" << step
                    << std::endl;
      ...
      for (int mIndex = 0; mIndex < htn->numMethodsForTask[task]; mIndex++) {
        ...
        if (htn->numSubTasks[method] == 0) {
          ...
          if (disjunct != edges[0][htn->subTasks[method][0]][to]) {
            ...
            // std::cout << "\tUnit: " << htn->subTasks[method][0] << " " <<
            // vertex_to_method[to] << std::endl;
          ...
          edges[methods_with_two_tasks_vertex[method]]
               [tasks_per_method[method].second][to] = sym_vars.zeroBDD();
          ...
          ensureBDD(tasks_per_method[method].first,
                    methods_with_two_tasks_vertex[method], sym_vars);
          BDD disjunct = edges[0][tasks_per_method[method].first]
                              [methods_with_two_tasks_vertex[method]] +
                         state;
          if (disjunct != edges[0][tasks_per_method[method].first]
                               [methods_with_two_tasks_vertex[method]]) {
            edges[0][tasks_per_method[method].first]
                 [methods_with_two_tasks_vertex[method]] = disjunct;

            // std::cout << "\t2 normal: " << tasks_per_method[method].first <<
            // " " << vertex_to_method[methods_with_two_tasks_vertex[method]] <<
            // std::endl;
            addQ(tasks_per_method[method].first,
                 methods_with_two_tasks_vertex[method]);
          }
        }
      }
